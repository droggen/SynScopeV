...
bool SSViewScatter::hasFocus() const
{
...
void SSViewScatter::setConfig(SSViewScatterConfig c)
{
   ...
   printf("SSViewScatter::setConfig. x,y,c size: %d %d %d\n",c.data_x.size(),c.data_y.size(),c.config.colors.size());



   plotwidget->setData(config.data_x,config.data_y,config.config.colors);

   plotwidget->setAutoscale(config.config.autoscale);
   plotwidget->setScale(config.config.xmin,config.config.ymin,config.config.xmax,config.config.ymax);
