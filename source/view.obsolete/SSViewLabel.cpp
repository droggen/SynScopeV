...
SSViewLabel::SSViewLabel(QWidget *parent) : SSViewAbstract(parent)
{
   ...
   //success=connect(plotwidget,SIGNAL(Repaint()),this,SLOT(on_Repaint()));
...
void SSViewLabel::on_Repaint()
{
   update();
}

void SSViewLabel::paintEvent(QPaintEvent *event)
{


//   plotwidget->GetPlot()->Plot();
...
void SSViewLabel::setConfig(SSViewLabelConfig c)
{
   ...
   /*DataLabel data;
   data.v = config.data;
   data.color = config.colors;
   plotwidget->GetPlot()->SetData(&data);*/
   plotwidget->setData(config.data,config.colors);
