...
WidgetScatter::WidgetScatter(QWidget *parent) :
...
WidgetScatter::~WidgetScatter()
{

}


PlotScatter *WidgetScatter::GetPlot()
{
    return &plot;
}

void WidgetScatter::resizeEvent(QResizeEvent *event)
{
    plot.Resize(event->size().width(),event->size().height());
    emit Repaint();
}
void WidgetScatter::paintEvent(QPaintEvent *event)
{
    plot.Plot();
    QPainter p(this);
...
*******************************************************************************
******************************************************************************/
void WidgetScatter::mouseMoveEvent ( QMouseEvent * event )
{
    emit mouseMoved(0);
    event->accept();
}

void WidgetScatter::mousePressEvent ( QMouseEvent * event )
{
